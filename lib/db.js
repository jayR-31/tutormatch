...
const DB_PATH = process.env.DB_PATH || path.join(DB_DIR, 'tutor-app.db');
  ...
  if (!db) {
    ...
    try {
      ...
      if (!fs.existsSync(dir)) {
        console.log(`[DB] Creating database directory: ${dir}`);
      ...
      throw error; // Rethrow to let the process crash/restart
    }
  }
  return db;
}
